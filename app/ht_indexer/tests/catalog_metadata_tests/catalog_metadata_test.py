...
@pytest.fixture()
def update_catalog_record_metadata(get_record_data: dict[str, Any]) -> dict[str, Any]:
...
@pytest.fixture()
def get_update_catalog_record_metadata(
    update_catalog_record_metadata: dict[str, Any],
...
@pytest.fixture()
def get_catalog_record_without_enum_pubdate(get_record_data: dict[str, Any]) -> dict[str, Any]:
...
@pytest.fixture()
def get_catalog_record_metadata_without_enum_pubdate(
    get_catalog_record_without_enum_pubdate: dict[str, Any],
) -> CatalogRecordMetadata:
...
class TestCatalogMetadata:
    def test_catalog_record_metadata_class(
        self, get_catalog_record_metadata: CatalogRecordMetadata
    ) -> None:
    ...
    def test_catalog_item_metadata_class(self, get_item_metadata: CatalogItemMetadata) -> None:
    ...
    def test_get_item_htsource(self, get_item_metadata: CatalogItemMetadata) -> None:
        htsource = get_item_metadata.get_item_htsource()
        assert htsource == "University of Michigan"

    def test_get_item_htsource_second_position(
        self, get_item_metadata_second_position: CatalogItemMetadata
    ) -> None:
    ...
    def test_get_item_htsource_sharinghtsource(
        self, get_item_metadata: CatalogItemMetadata
    ) -> None:
        htsource = get_item_metadata.get_item_htsource()
        assert htsource == "University of Michigan"

    def test_get_volume_enumcron_empty(self) -> None:
        """
    ...
    def test_missed_enum_publish_date(
        self, get_item_metadata_without_enum_pubdate: CatalogItemMetadata
    ) -> None:
        doc_json = get_item_metadata_without_enum_pubdate.get_data_ht_json_obj()
        assert len(doc_json) == 0

    def test_extract_enum_publish_date(self, get_item_metadata: CatalogItemMetadata) -> None:
