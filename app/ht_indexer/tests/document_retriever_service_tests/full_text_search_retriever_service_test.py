...
def get_document_retriever_service(
    ...
    return FullTextSearchRetrieverQueueService(
        get_queue_config[0].queue_params,
        solr_catalog_url,
        "solr_user",
        "solr_password",
...
class TestFullTextRetrieverService:
    ...
    def test_full_text_service_retrieve_documents_from_solr(
        ...
        response = get_document_retriever_service.retrieve_documents_from_solr(
            query, get_solr_request
        )
    ...
    def test_retrieve_documents_by_item(self, get_solr_request, get_document_retriever_service):
        ...
        record_metadata_list = FullTextSearchRetrieverQueueService.generate_chunk_metadata(
            list_documents, output, by_field
        )
...
class TestMainSerialBranch:
    ...
    def test_serial_branch_passes_extracted_ids_not_raw_mysql_rows(self):
        ...
        with (
            patch.object(
                retriever_service_module, "RetrieverServiceArguments", return_value=mock_args
            ),
            patch.object(
                retriever_service_module,
                "FullTextSearchRetrieverQueueService",
        ...
        _, documents_arg, by_field_arg = (
            mock_service_instance.full_text_search_retriever_service.call_args.args
        )
