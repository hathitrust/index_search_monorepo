...
class TestChannelFactory:
    ...
    def test_create_channel_close(self, rabbit_mq_connection, get_rabbit_mq_host_name):
        ...
        channel_creator = ChannelCreator(rabbit_mq_connection)
    ...
    def test_purge_queue(self, get_rabbit_mq_host_name):

        queue_name = "test_purge_queue"

        # Create producer instance
        queue_producer = QueueProducer("guest", "guest", get_rabbit_mq_host_name, queue_name)

        if not queue_producer.queue_setup.is_ready():
        ...
        # assert status.method.message_count >= 1
        ...
        queue_producer.queue_connection.close()