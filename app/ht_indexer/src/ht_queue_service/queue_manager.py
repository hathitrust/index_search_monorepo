...
class QueueManager:
    ...
    def __init__(self, config: FlexibleDict):
        ...
        * batch_size: The maximum number of unacknowledged deliveries that are permitted on
        ...
        self.main_exchange_name = config["main_exchange_name"]
        self.dlx_exchange = config["dlx_exchange"]
        self.exchange_type = config["exchange_type"]
        self.durable = config.get("durable", True)  # Default to True if not specified
        self.auto_delete = config.get("auto_delete", False)  # Default to False
    ...
    def set_up_queue(self, channel: Channel=None) -> None:
        ...
        ch.queue_bind(self.dead_letter_queue_name, self.dlx_exchange, self.arguments["x-dead-letter-routing-key"]) #f"dlx_key_{self.queue_name}"
    ...
    def get_total_messages(self, channel: pika.adapters.blocking_connection.BlockingChannel) -> Any:
