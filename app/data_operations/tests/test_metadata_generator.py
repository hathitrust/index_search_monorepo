...
def write_sample(path: Path, records) -> Path:
    payload = "\n".join(json.dumps(record) for record in records)
    path.write_bytes(gzip.compress(payload.encode("utf-8")))
    return path


def test_record_matches_detects_dissertation_keywords():
...
def test_extract_identifiers():
    ...
    record = Record()
    record.add_field(Field(tag="035", indicators=[" ", " "], subfields=[Subfield(code="a", value="(ProQuest)disstheses AAI999")]))
    record.add_field(Field(tag="502", indicators=[" ", " "], subfields=[Subfield(code="o", value="AAI8999")]))
    record.add_field(Field(tag="035", indicators=[" ", " "], subfields=[Subfield(code="a", value="(MiU)990027275210106381")]))
...
def test_generate_dissertation_rows(tmp_path: Path):
