...
def main():
    ...
    parser.add_argument(
        "--file_name", help="Name od the file to load the list of ids", required=True
    )
    ...
    solr_exporter = SolrExporter(
        solr_base_url, args.env, user=os.getenv("SOLR_USER"), password=os.getenv("SOLR_PASSWORD")
    )
    # '"good"'
    list_documents = []
    for x in solr_exporter.run_cursor(
        query, query_config_file_path, conf_query=conf_query, list_output_fields=["id", "ht_id"]
    ):
        dict_x = json.loads(x)
        if "ht_id" in dict_x:
