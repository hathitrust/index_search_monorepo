...
if __name__ == "__main__":
    ...
    for engine in engines:
        ...
        for query in list_queries:
            ...
            a_path = (
                f"{query['query_fields']}_{query['query_string']}_{query['operator']}_{engine}.csv"
            )
    ...
    # Merge results
    df = list_hits_results[0].merge(
        list_hits_results[1], on=["query_string", "operator"], suffixes=("_solr6", "_solr8")
    )
    df.to_csv(path_or_buf="solr6_y_solr8_hits_16908_documents.csv", index=False, sep="\t")
