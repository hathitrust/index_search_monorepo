...
statistics_fields = {
    "language008_full": "Language dist all",
...
def map_callnoletters(df: pd.DataFrame, map_file_path: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    with open(map_file_path, encoding="latin-1") as file:
    ...
    grouped_df["mapped_callnoletters"] = grouped_df["mapped_callnoletters"].apply(
        lambda x: get_category_name(x)
    )
        ...
        for key, values in solr_facets_output.items():
            ...
            if key == "bothPublishDateRange":
                ...
                df_callnoletters.to_excel(
                    writer, sheet_name=f"{statistics_fields[key]}_Callnoletters", index=False
                )
                df_general.to_excel(
                    writer, sheet_name=f"{statistics_fields[key]}_General", index=False
                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--map_file_path",
        help="Path to the map_call_number.properties file",
        default="/".join([os.getcwd(), "map_call_number.properties"]),
    )
    ...
    # Create query object
    Q = HTFullTextQuery(config_query=args.query_config, config_facet_field=args.facet_config)
    ...
    solr_output = ht_full_search.solr_facets_output(
        query_string=None, fl=fl, operator=args.operator
    )
