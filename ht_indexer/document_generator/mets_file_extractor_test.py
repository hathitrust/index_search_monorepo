...
class TestMETSFileExtractor():
    ...
    def test_create_METS_map(self, metsAttrExtractorObj):
        ...
        assert (mets_map.get('488').get('pgnum') == None)
        assert (mets_map.get('488').get('filename') == ["IMG00000488", "HTML00000488", "TXT00000488"])
    ...
    def test_get_unique_features(self, metsAttrExtractorObj):
        ...
        assert sorted(["CHAPTER_START",
                       "FIRST_CONTENT_CHAPTER_START",
                       "UNTYPICAL_PAGE",
                       "FRONT_COVER",
                       "IMPLICIT_PAGE_NUMBER"]) == sorted(MetsAttributeExtractor.get_unique_features(mets_map))
