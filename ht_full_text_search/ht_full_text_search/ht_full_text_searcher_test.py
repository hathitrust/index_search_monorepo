...
class TestHTFullTextSearcher():
    ...
    def test_search(self, ht_full_text_query):
        searcher = HTFullTextSearcher(engine_uri=config_search.SOLR_URL["dev"],
                                      ht_search_query=ht_full_text_query,
                                      use_shards=False)
        solr_results = searcher.solr_result(query_string="majority of the votes", fl=["author", "id", "title"], operator="AND")
